"""PIBO recalib v8: pure Nelder-Mead local refinement from v6 trial 137 best
(17.66%). Wider bounds upward on params that hit v6 ceiling. Then Powell.

Why NM: v6 GP-EI plateau'd at 17.66% for 105 trials; v7 DE plateau'd for 100
trials. Both lost the narrow basin. NM samples single-direction perturbations
from a known-good simplex — robust to noisy LAMMPS objective.

Param set: same 19 as v6 (no atom-level expansion — keeps simplex tractable).
"""
...
SRC_FFIELD = ROOT / "results" / "ffield" / "ffield.reax.MoSH.pibo_biaxial_v6.reax"
DST_FFIELD = ROOT / "results" / "ffield" / "ffield.reax.MoSH.pibo_biaxial_v8.reax"
...
OUT_DIR    = ROOT / "results" / "reviewer_response" / "biaxial_recalib_v8"
LOG_PATH   = OUT_DIR / "RECALIB_V8_LOG.csv"
...
# v6 17.66% best hit ceilings on De_sigma, De_pi, p_val1_SMoS.
# v8 widens upper bounds on these; tighter on others (we're refining).
WIDEN = {
    "De_sigma_MoS":  (0.60, 2.50),   # init=30.75 → range[18.45, 76.88]
    "De_pi_MoS":     (0.60, 2.50),   # init=113.02 → [67.81, 282.55]
    "p_val1_SMoS":   (0.50, 2.50),   # init=134.02 → [67.01, 335.05]
    "p_val1_MoSMo":  (0.50, 2.50),
    "Thetao_SMoS":   (0.90, 1.20),
    "Thetao_MoSMo":  (0.90, 1.20),
    "p_be1_MoS":     (0.50, 2.00),
    "p_bo1_MoS":     (0.50, 1.50),
    "p_bo2_MoS":     (0.60, 1.50),
    "p_bo5_MoS":     (0.50, 1.50),
    ...
    "RvdW_MoS":      (0.80, 1.30),
    "Alfa_MoS":      (0.50, 1.80),
    "ro_sigma_MoS":  (0.90, 1.15),
    "p_val2_SMoS":   (0.50, 1.80),
    "p_val7_SMoS":   (0.50, 1.80),
    "p_val4_SMoS":   (0.50, 1.80),
}
...
def build_spec(lines, off):
    ...
    for s in spec:
        ...
        lo_m, hi_m = WIDEN.get(s["name"], (0.5, 2.0))
...
def main():
    ...
    print(f"[setup] v8: src={SRC_FFIELD.name}, {n_params} params, "
          f"NM->Powell, target<{TARGET_MAX_RE}%", flush=True)
    print(f"[setup] x0 ranges OK: {(x0 >= lows).all() and (x0 <= highs).all()}", flush=True)
    ...
    def objective(x):
        # Project onto bounds (NM/Powell may step outside)
    ...
    for i in range(n_params):
        step = 0.05 * abs(x0[i]) if abs(x0[i]) > 1e-3 else 0.01
        if x0[i] >= 0:
            initial_simplex[i + 1, i] = min(highs[i], x0[i] + step)
        else:
            initial_simplex[i + 1, i] = max(lows[i], x0[i] - step)

    # Phase 1: Nelder-Mead bounded, modest initial simplex
    print(f"[phase NM] starting from bootstrap (small simplex, 5% step)", flush=True)
    try:
        minimize(objective, x0, method="Nelder-Mead",
                 bounds=bounds_pairs,
                 options={"maxiter": 1500, "xatol": 1e-4, "fatol": 1e-2,
    ...
    # Phase 2: Powell from current best
    ...
    # Phase 3: NM restart with larger simplex (escape local minimum)
    if best["max_re"] > TARGET_MAX_RE and best["params"] is not None:
        print(f"[phase NM2] larger simplex from best={best['max_re']:.2f}%", flush=True)
        phase[0] = "NM2"
        xb = np.array(best["params"])
        simplex2 = np.tile(xb, (n_params + 1, 1)).astype(float)
        for i in range(n_params):
            step = 0.20 * abs(xb[i]) if abs(xb[i]) > 1e-3 else 0.05
        ...
        try:
            minimize(objective, xb, method="Nelder-Mead",
                     bounds=bounds_pairs,
                     options={"maxiter": 2000, "xatol": 1e-4, "fatol": 1e-2,
