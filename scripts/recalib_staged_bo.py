"""Staged Bayesian Optimization for 5-priority ReaxFF recalibration with
Thompson sampling injection and full posterior data collection.

Acquisition strategy (per goal update 2026-05-25):
    * Stage 1 (trials 1 → STAGE1_END=15): LHS exploration (cold start)
    * Stage 2 (trials 16 → STAGE2_END=60): EI 주체, p=0.15 Thompson sampling
    * Stage 3 (trials 61 → budget): LCB exploitation

Each ask records:
    - acquisition type used (LHS, EI, Thompson, LCB)
    - candidate GP mean/std at the chosen point
    - 0/1 Thompson-coin (if applicable)

Every N_POST_DUMP trials, snapshots the GP posterior at LHS grid points
(default 200 points × 5 dims) for downstream posterior-distribution figures.

Output (results/reviewer_response/recalib_staged_bo/):
    RECALIB_LOG.csv           per-trial all 5 priority metrics + h_S + params + acq_type
    BEST_RESULT.txt
    ffield.reax.MoSH.staged_bo.reax  (best so far)
    posterior_snapshots/      per-snapshot CSV: trial, x_grid, gp_mean, gp_std
"""
...
# Per user hint 2026-05-25: v9 reached biaxial 9.19 % on OLD reference and
# evaluates to biax=30.7 %, x1σ=65.6 %, x2σ=20.8 %, V_S=33 % on the CORRECTED
# reference — substantially better than PIBO calibrated (106/85/43/27 %).
# Warm-start from v9 to give the staged BO a much closer initial point.
SRC_FFIELD = ROOT / "results" / "ffield" / "ffield.reax.MoSH.pibo_biaxial_v9.reax"
DST_FFIELD = ROOT / "results" / "ffield" / "ffield.reax.MoSH.staged_bo.reax"
...
OUT_DIR    = ROOT / "results" / "reviewer_response" / "recalib_staged_bo"
...
# Reuse the same WIDEN bounds and spec builders as recalib_combined_all.py
# Import directly so we don't drift.
