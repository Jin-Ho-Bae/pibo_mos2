"""PIBO recalib v13: constrained physics-informed Bayesian Optimization
...
DST_FFIELD = ROOT / "results" / "ffield" / "ffield.reax.MoSH.pibo_biaxial_v13.reax"
...
OUT_DIR    = ROOT / "results" / "reviewer_response" / "biaxial_recalib_v13"
LOG_PATH   = OUT_DIR / "RECALIB_V13_LOG.csv"
BEST_PATH  = OUT_DIR / "BEST_RESULT.txt"
TARGET_RE  = 10.0   # target % rel err on every individual observable

# v13 constrained-loss tolerances. Geometry constraints behave as soft
...
# v13: bootstrap from v11 best (Pareto point: biax=52 %, geometry within
...
WIDEN = {
    ...
    # v13: atom-level Mo handles -> control strain stiffening, over-coordination
    "Mo_RvdW":       (0.92, 1.08),
    "Mo_Dij":        (0.70, 1.40),
    "Mo_gamma":      (0.85, 1.18),
    "Mo_Alfa":       (0.85, 1.18),
    "Mo_gamma_w":    (0.80, 1.25),
    "Mo_p_boc4":     (0.60, 1.60),
    "Mo_p_boc3":     (0.50, 2.00),
    "Mo_p_boc5":     (0.60, 1.60),
    "Mo_p_ovun2":    (0.70, 1.40),
    "Mo_p_val3":     (0.70, 1.40),
    "Mo_p_val5":     (0.80, 1.25),
    # v13: atom-level S handles
    "S_RvdW":        (0.94, 1.06),
    "S_Dij":         (0.75, 1.30),
    "S_gamma":       (0.88, 1.15),
    "S_Alfa":        (0.88, 1.15),
...
def build_spec(lines, off):
    ...
    # v13 ADDITIONS: atom-level Mo and S parameters that govern strain
    # stiffening and over-coordination without changing equilibrium bond
    # length. Each atom occupies 4 lines of 8 floats; Mo at atom_start+0..3,
    # S at atom_start+4..7. The leading token is the element symbol.
    atom_start = off["atom_start"]
    # Mo line 1 (cols: ro_sigma, Val, mass, RvdW, Dij, gamma, ro_pi, Val_e)
    mo_l1 = atom_start; mo_l2 = atom_start + 1
    mo_l3 = atom_start + 2; mo_l4 = atom_start + 3
    s_l1  = atom_start + 4; s_l2  = atom_start + 5
    s_l3  = atom_start + 6; s_l4  = atom_start + 7

    def _parse_floats(ln_idx):
        toks = lines[ln_idx].split()
        # Strip element symbol if present
    ...
    # Mo line 1: ro_sigma(0), RvdW(3), Dij(4), gamma(5)
    ...
    # Mo line 4: p_ovun2(0), p_val3(1), p_val5(4)
...
def evaluate(spec, x, src_lines, dft_eps, dft_sig):
    ...
    # v13 constrained quadratic-penalty loss. Biaxial is the primary
