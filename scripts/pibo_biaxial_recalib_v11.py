"""PIBO recalib v10: joint physics-informed Bayesian Optimization that
constrains BOTH biaxial sigma(epsilon) accuracy AND equilibrium geometry
(a, d_MoS, angle_intra) simultaneously.

Differences from v9 (which traded geometry for sigma(eps)):
  - 22 parameters bootstrapped from v9 best
  - Asymmetric bounds: tight around bond/angle handles, wide on RvdW/Alfa/
    ro_sigma/Dij/Thetao_* that govern equilibrium geometry
  - **Joint max-aggregation loss**: max(biaxial_re, weighted_a_re,
    weighted_d_re, weighted_ang_re).  All four must stay <= 10% for the
    target to be met.
...
FB_FFIELD  = ROOT / "results" / "ffield" / "ffield.reax.MoSH.pibo_biaxial_v9.reax"
if not SRC_FFIELD.exists(): SRC_FFIELD = FB_FFIELD
DST_FFIELD = ROOT / "results" / "ffield" / "ffield.reax.MoSH.pibo_biaxial_v11.reax"
...
OUT_DIR    = ROOT / "results" / "reviewer_response" / "biaxial_recalib_v11"
LOG_PATH   = OUT_DIR / "RECALIB_V11_LOG.csv"
...
WIDEN = {
    # Bond depth / curvature handles: wide -> chase biaxial sigma(eps)
    "De_sigma_MoS":  (0.80, 1.25),
    "De_pi_MoS":     (0.80, 1.25),
    "p_be1_MoS":     (0.60, 1.60),
    "p_be2_MoS":     (0.50, 1.80),
    "p_bo1_MoS":     (0.70, 1.35),
    ...
    "p_bo5_MoS":     (0.60, 1.50),
    "p_bo6_MoS":     (0.50, 1.80),
    "p_ovun1_MoS":   (0.50, 1.80),
    # Off-diagonal vdW / radius: tighter to preserve good equilibrium
    "Dij_MoS":       (0.85, 1.20),
    "RvdW_MoS":      (0.94, 1.08),
    "Alfa_MoS":      (0.90, 1.15),
    "ro_sigma_MoS":  (0.96, 1.05),
    # Angle terms: tight (PIBO calibrated already nails angles)
    "Thetao_SMoS":   (0.96, 1.05),
    "Thetao_MoSMo":  (0.93, 1.10),
    "p_val1_SMoS":   (0.85, 1.20),
    "p_val1_MoSMo":  (0.80, 1.25),
    "p_val2_SMoS":   (0.80, 1.30),
    "p_val7_SMoS":   (0.80, 1.30),
    "p_val4_SMoS":   (0.80, 1.30),
}
...
def evaluate(spec, x, src_lines, dft_eps, dft_sig):
    ...
    # Joint loss: weighted max -> all four must drop together
    joint = max(W["biaxial"] * biax_max_re,
                W["a"]       * a_re,
                W["d_MoS"]   * d_re,
                W["angle_intra"] * ang_re)
...
def main(budget=80, seed=42):
    ...
    for trial in range(1, budget + 1):
        ...
        if best["joint"] <= TARGET_RE:
            print(f"\n[target met!] best joint = {best['joint']:.3f} <= {TARGET_RE}")
            break
