"""LAMMPS evaluator + ReaxFF text-format helpers for the PIBO recalibration.

In this bundle, this module is imported as a LIBRARY by
``recalib_staged_bo.py`` (the entry point). It provides:

  - ReaxFF ffield parsing/writing: ``read_lines``, ``parse_offsets``,
    ``build_spec`` (the 40 BO-controlled parameters + bounds), ``write_ffield``.
  - LAMMPS input decks: biaxial / uniaxial stress, vacancy, saddle.
  - ``evaluate(spec, x, src_lines, refs, wd_root)`` — runs LAMMPS and returns
    the relative error of each observable vs DFT.

It targets seven observables simultaneously, with the optimization loss being
their worst-of-N relative error (target: every metric <= 10 %):

  1. biaxial sigma(eps)          5. uniaxial x2 h_S
  2. uniaxial x1 sigma           6. V_S vacancy formation energy
  3. uniaxial x2 sigma           7. S diffusion barrier (Path 2)
  4. uniaxial x1 h_S

Running this file directly (``python recalib_combined_all.py``) performs the
standalone 7-objective recalibration; the canonical entry point for the
manuscript run is ``recalib_staged_bo.py``. LAMMPS is required (see _lmp_path).
"""
...
# Self-contained bundle layout: inputs in data/, results in output/.
HERE = Path(__file__).resolve().parent
DATA = HERE / "data"
SRC_FFIELD = DATA / "ffield.reax.MoSH.pibo_biaxial_v9.reax"   # warm-start (input)
DST_FFIELD = HERE / "output" / "ffield.reax.MoSH.combined_recalib.reax"
...
OUT_DIR    = HERE / "output" / "recalib_combined"
